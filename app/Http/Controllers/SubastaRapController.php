...
class SubastaRapController extends Controller
{
    ...
    public function pagoVendedor(){
        ...
        $subastaTerminadam = Producto::where('final_subasta','<',$fecha)->where('user_id_comprador','=',auth()->user()->id)->latest()->first()->get();  
        dd($subastaTerminadam[6]->id);

        $pujasMezclas = DB::table('pujas')->where('user_id_comprador','=',auth()->user()->id);
